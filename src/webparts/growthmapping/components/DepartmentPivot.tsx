...
export default function DepartmentPivot() 
{
    ...
    async function getdata() 
    {
           ...
           .then(function(data)
           {
               ...
               {  
                  ...
                  for(let i=0;i<data.length;i++)
                  {
                    let index=listdata.findIndex(o => o.Department == data[i].Department)
                    listdata[index].Details.push({ID:data[i].ID,Position:data[i].Position,Department:data[i].Department}); 
                  }
  ...
  async function getDatafromsharepoint(itemID)
  {
    ...
    {   
        ...
        if(data)
        {   rows=[];
            rows.push({Department:data.Department,Position:data.Position,BehavioralTraits:data["BehavioralTraits"],TechnicalTraits:data["TechnicalTraits"],RoleDesciption:data["RoleDesciption"]});
    ...
    return (<div className='clsgrowthmapping'>
    <TreeView
    ...
    >
      {items.map(function(item,index){
        return(<TreeItem nodeId={index.toString()} label={item.Department}>
            {item.Details.map(function(seconditem,index)
            {
                let labelvalue=seconditem.Position;
                return(<TreeItem nodeId={items.length.toString()} label={labelvalue} onClick={()=>
                {
    ...
    <DetailPopup modal={modalopen} itemdetails={selecteditem} Department={selectedDept} closeclick={modalclose}/>
    </div>
  );
}